...
export const handleQcSearch = async (
  ...
  channel?: number,
  offerId?: string,
  searchByChannel: boolean = false 
) => {
  try {

    // 🔹 OPTION 1: Search using channel + offerId if toggle is active
    if (searchByChannel && channel && offerId) {
      try {
        const response = await axios.post(
          "https://api.onlyfinds.ai/partner-api/get/goods/qc/list",
          {
            appKey: "DlZhchwAgigDhYAl71Ba0KNqLz2ciGQS",
            appId: "5412377",
            channel: channel,
            offerId: offerId,
          },
        ...
        if (response.data?.data) {
          ...
          Swal.fire({
            icon: "error",
            title: "No products found!",
            text: "No result found with channel and offerId combination.",
          });
          return;
        }
      } catch (error) {
        console.error("OnlyFinds API (channel+offerId) error:", error);
        Swal.fire({
          icon: "error",
          title: "Search Failed",
          text: "Search by channel and offerId failed. Try again.",
        });
        return;
      }
    }
    ...
    // 🔹 OPTION 2: Normal URL-based search (existing logic)
